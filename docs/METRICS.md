...
| G8 (2026-09-16) | Strike positions | not available | `/lightning`: 331 cells / 882 flashes around Cincinnati during the evening's storms, ~14 KB, served from memory (60 s cache per 0.2° cell); first poll after start pulls ~90 files (15 min), then ~6 files a minute | Upstream: NOAA S3, anonymous. GOES-East files ~360 KB, GOES-West ~220 KB, every 20 s each: about 2.5 GB/day into Tower, fixed. Image +~40 MB (h5py + numpy). Per user: zero upstream. |
